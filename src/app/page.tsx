...
export default function Home() {
  ...
  const [allTracksCache, setAllTracksCache] = useState<Track[]>([]);
  ...
  useEffect(() => {
    if (tracks.length > 0 && allTracksCache.length === 0) {
      setAllTracksCache(tracks);
    }
  }, [tracks, allTracksCache]);

  // Function to perform the actual search (without debounce)
  const performSearch = useCallback(
    (query: string) => {
      ...
      try {
        // Search in the cached tracks instead of fetching again
        const filteredResults = allTracksCache.filter(
          (track) =>
    ...
    [allTracksCache, setLoading, setError, setSearchResults, setIsSearching]
  );
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      // Directly perform search without debounce
            ...
            <div className="pt-6">
              <div className="bg-gradient-to-r from-green-600/30 to-purple-600/30 rounded-xl p-4">
                ...
                <p className="text-sm text-gray-300 mb-3">
                  Save tracks and create your own personalized playlists
                </p>
        ...
        <div className={`px-6 md:px-10 ${isIOS ? "pb-52" : "pb-48"} lg:pb-32`}>
          ...
          {isLoading ? (
            ...
            <div className="bg-white/5 rounded-lg overflow-hidden mt-4">
              ...
              {/* "No tracks" message when there are no tracks to display */}
